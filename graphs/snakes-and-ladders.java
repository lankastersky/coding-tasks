...
*/
  ...
  class Solution {
    ...
    public int snakesAndLadders(int[][] board) {
      ...
      int[] line = buildLine(board);
//      int[] memo = new int[line.length];
//      boolean[] visited = new boolean[line.length];
//      Arrays.fill(memo, -1);
//      int res = move(line, 0, 0, visited, memo);
...
//      }
    ...
    int[] buildLine(int[][] board) {
      ...
      for (int i = 0; i < m; i++) {
        ...
        int j = 0;
        while (j < n) {
          line[i * n + j] = board[m - i - 1][toRight ? j : n - j - 1];
          j++;
        }
      }
      return line;
...
//    int move(int[] line, int pos, int moves, boolean[] visited, int[] memo) {
//      int n = line.length;
//      if (pos == n - 1) {
//        return moves;
//      }
...
//          break;
//        }
//        int dest = line[pos + s] == -1 ? pos + s : line[pos + s] - 1;
//        if (visited[dest]) {
//          continue;
//        }
//        visited[dest] = true;
//        int temp = move(line, dest, moves + 1, visited, memo);
    ...
    int bfs(int[] line) {
      ...
      while (!queue.isEmpty()) {
        ...
        for (int i = 0; i < size; i++) {
          ...
          for (int j = pos + 1; j <= Math.min(n - 1, pos + STEPS); j++) {
            int dest = line[j] == -1 ? j : line[j] - 1;
            if (visited[dest]) {
              continue;
            }
            visited[dest] = true;
