...
class Solution {
  public String findReplaceString(String S, int[] indexes, String[] sources, String[] targets) {
    ...
    Arrays.sort(sorted, (i, j) -> {
      return indexes[i] - indexes[j];
    });
    for (int j = 0; j < n; j++) {
      int i = sorted[j];
      int ind = indexes[i];
      if (!S.substring(ind).startsWith(sources[i])) {
        continue;
      }
      int start = ind + shift;
      int end = ind + sources[i].length() + shift;
      sb.replace(start, end, targets[i]);
      shift = shift + targets[i].length() - sources[i].length();
    }
    return sb.toString();
  }
}
