...
class Solution {
  public int[] exclusiveTime(int n, List<String> logs) {
    ...
    for (String log : logs) {
      ...
      int id = Integer.valueOf(task[0]);
      String mode = task[1];
      int time = Integer.valueOf(task[2]);
      if (mode.equals("start")) {
        // pause previos task
        if (!stack.isEmpty()) {
          int prevId = Integer.valueOf(stack.peek());
          res[prevId] += time - prevTime;
        }
        stack.push(id);
        prevTime = time;
      } else if (mode.equals("end")) {
        // we can only stop currently running task which is on top of stack
