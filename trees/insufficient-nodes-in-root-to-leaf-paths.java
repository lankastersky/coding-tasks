 ...
 */
 ...
 // There is a simpler solution, see
...
class Solution {
  public TreeNode sufficientSubset(TreeNode root, int limit) {
    if (insuf(root, limit, 0)) {
      return null;
    }
    return root;
  }

  boolean insuf(TreeNode root, int limit, int sum) {
    if (root == null) {
      return true;
    }
    sum += root.val;
    ...
    boolean leftres = insuf(root.left, limit, sum);
    if (leftres) {
      root.left = null;
    }
    boolean rightres = insuf(root.right, limit, sum);
    if (rightres) {
      root.right = null;
    }
