...
We can model each transaction as a tuple (x, y, z) which means person x gave person 
y $z. Assuming Alice, Bill, and Chris are person 0, 1, and 2 respectively (0, 1, 2 
...
Person's IDs may not be linear, e.g. we could have the persons 0, 1, 2 or we could 
...
class Solution {
  // Based on https://leetcode.com/problems/optimal-account-balancing/discuss/95355/11-liner-9ms-DFS-solution-(detailed-explanation)
  ...
  int settle(List<Integer> debt, int start) {
    int n = debt.size();
    ...
    for (int i = start + 1; i < n; i++) {
      if (debt.get(start) * debt.get(i) < 0) {
        //debt.set(start, debt.get(start) + debt.get(i));
        debt.set(i, debt.get(i) + debt.get(start));
        res = Math.min(res, 1 + settle(debt, start + 1));
        //debt.set(start, debt.get(start) - debt.get(i));
        debt.set(i, debt.get(i) - debt.get(start));
      }
    }
