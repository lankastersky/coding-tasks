...
class Solution {
  public int assignBikes(int[][] workers, int[][] bikes) {
    ...
    return dfs(workers, bikes, 0, visited, dp);
  }
  
  int dfs(int[][] workers, int[][] bikes, 
          int pos, boolean[] visited, Map<Integer, Integer> dp) {
    int n = workers.length;
    if (pos == n) {
      return 0;
    }
    int key = buildKey(visited);
    ...
    for (int i = 0; i < bikes.length; i++) {
      ...
      visited[i] = true;
      int dist = dist(workers[pos], bikes[i]);
      res = Math.min(res, dist + dfs(workers, bikes, pos + 1, visited, dp));
      visited[i] = false;
    }
    dp.put(key, res);
    return res;
